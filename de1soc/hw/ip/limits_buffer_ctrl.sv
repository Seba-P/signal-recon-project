...
(
  ...
  input  wire        iter_output_enable,
  /* Hardlimiter IF */
  output wire [31:0] limiter_data,
  output wire        limiter_valid,
  /* Limits buffer IF */
...
begin
  ...
  begin
    ...
    ram_limits_write_r      <= '(iter_input_enable & lvl_gen_valid);
    ram_limits_writedata_r  <= lvl_gen_limits;
    ram_limits_byteenable_r <= '1;

    symbol_cnt_r            <= full_buffer ? 'd0 : symbol_cnt_r + '(iter_input_enable & lvl_gen_valid); // ?
  end
end
