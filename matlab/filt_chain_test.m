...
% Signal parameters
f_sig       = 4400;
signal      = 4 + 3.2*sin(2*pi*f_sig*t) + 2*sin(3*2*pi*f_sig*t);
...
LVLS        = fftshift(fft(lvls));
SIGNAL      = fftshift(fft(signal_s));
freq_s      = linspace(-fs_s/2, fs_s/2, N_sig_s);
%LVLS_A     = fftshift(fft(lvls_a));
%SIGNAL_A   = fftshift(fft(signal_a));
%FILT_LVLS  = fftshift(fft(filt_lvls_1));
%FILT_SIG   = fftshift(fft(filt_sig_1));
...
%fig2 = subplot(2, 1, 2);
%hold(fig1, 'on');
%plot(fig2, t_a, abs(LVLS_A), 'r')
%plot(fig2, t_a, abs(SIGNAL_A), 'b')

%figure
%hold on
%plot(t1, abs(FILT_LVLS), 'r')
%plot(t1, abs(FILT_SIG), 'b')
